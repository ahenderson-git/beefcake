...
export class PipelineLibrary {
  ...
  async init(): Promise<void> {
    await Promise.all([this.loadPipelines(), this.loadTemplates()]);
    this.render();
  ...
  private async loadPipelines(): Promise<void> {
    try {
      ...
      this.state.error = `Failed to load pipelines: ${String(error)}`;
  ...
  private async loadTemplates(): Promise<void> {
    try {
      this.state.templates = await listTemplates();
    } catch (error) {
      this.state.error = `Failed to load templates: ${String(error)}`;
    }
  }
  ...
  private getFilteredPipelines(): PipelineInfo[] {
    ...
    return this.state.pipelines.filter(
      p =>
  ...
  render(): void {
    const items =
      this.state.viewMode === 'pipelines' ? this.getFilteredPipelines() : this.state.templates;
  ...
  private renderPipelineCard(pipeline: PipelineInfo): string {
    const created = pipeline.created ? new Date(pipeline.created).toLocaleDateString() : 'Unknown';
  ...
  private getTemplateCategory(name: string): string {
    ...
    if (name.includes('Date') || name.includes('Type') || name.includes('Text'))
    ...
    this.container.querySelectorAll('.use-template-btn').forEach(btn => {
      btn.addEventListener('click', e => {
        void (async () => {
          ...
          if (templateName) await this.handleUseTemplate(templateName);
        })();
      });
    });
  }
  ...
  private async handleUseTemplate(templateName: string): Promise<void> {
    try {
      ...
      alert(`Failed to load template: ${String(error)}`);
    }
  }
