...
export class PipelineComponent extends Component {
    ...
    render(_state: AppState): void {
        try {
            ...
            if (this.viewMode === 'library') {
                ...
                const libraryContainer = container.querySelector('#pipeline-library-container') as HTMLElement;
                if (libraryContainer) {
                    this.initialisePipelineLibrary(libraryContainer);
                }
            } else if (this.viewMode === 'editor') {
                // Show editor view
                container.innerHTML = `<div id="pipeline-editor-container"></div>`;
                const editorContainer = container.querySelector('#pipeline-editor-container') as HTMLElement;
                if (editorContainer) {
                    this.initialisePipelineEditor(editorContainer);
                }
            }
        } catch (error) {
    ...
    private async initialisePipelineEditor(editorContainer: HTMLElement): Promise<void> {
        ...
        if (this.currentPipelinePath) {
            try {
                ...
                this.actions.showToast(`Failed to load pipeline: ${error}`, 'error');
                this.showLibrary();
                return;
            }
        ...
        this.pipelineEditor.setOnSave(async (spec: PipelineSpec) => {
            await this.handleSavePipeline(spec);
        });
    ...
    private async handleEditPipeline(path: string): Promise<void> {
    ...
    private async handleSavePipeline(spec: PipelineSpec): Promise<void> {
        try {
            ...
            this.actions.showToast(`Failed to save pipeline: ${error}`, 'error');
    ...
    private handleExecutePipelineFromSpec(spec: PipelineSpec): void {
        try {
            ...
            this.actions.showToast(`Failed to execute pipeline: ${error}`, 'error');
